...
class OrdinalSlipNoise(NoiseModel):
    def corrupt(self, label, labels, item_index=0):

        if self.rng.random() >= self.rate: return label
        i=labels.index(label); return labels[max(0,min(len(labels)-1,i+self.rng.choice([-1,1])))]
